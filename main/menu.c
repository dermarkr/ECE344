...
//	}
	...
	/*
	struct box * cargo = (struct box *)nargs; 
    curthread->t_vmspace = cargo->as; 

	curthread->pid = cargo->pid;
    curthread->active = 1;
    parents[cargo->pid] = cargo->parent;

	int temp = cargo->active;
	nargs = temp;

	threadarray[curthread->pid] = curthread;
	*/
	/* Hope we fit. */
...
cmd_dbflags(int n, char **a){
    ...
    if(dbflags < 16){
        kprintf("Current value of dbflags is 0x%01x \n", dbflags);
    }else{
        kprintf("Current value of dbflags is 0x%02x \n", dbflags);
    }
...
updateDB(int n, char **a){
    ...
    //kprintf("Yahhhhhhh %d %s%d%s \n", dbflags, command, num, state);
    return 0;
}
